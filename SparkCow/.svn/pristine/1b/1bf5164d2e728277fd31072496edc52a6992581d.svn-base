...
//        StringBuilder out;
    ...
    // md5(screct_key + id+ type + ver + time )
    std::string generateSignature(const std::string& secret_key, const std::string& text ){
        std::string sign;
        std::string data = secret_key + text + secret_key;
//        sign = mongo::md5simpledigest(data);
        return sign;
    }


    std::string getDeviceUniqueId(){
        const char* device_file = "/dev/mmcblk0p8";
        std::string uid ;
        FILE * fp = fopen(device_file, "rb");
        if(fp){
            char buff[256+1];
        ...
        std::ifstream file("/etc/default/ssdd.conf");

        // SN:FBBFGW1910XXXX0000000011
        if (file.is_open())
        {
            std::string line;
    ...
    bool writeDeviceUniqueId(const std::string& uid) {
        ...
        if(fp){
            char buff[256+1];
            memset(buff,0,sizeof(buff));
            memcpy(buff,uid.c_str(),uid.size());
...
//        }
    ...
    void toByteArray(const char * data, size_t size , ByteArray& bytes){
        std::copy(data,data+size,std::back_inserter(bytes));
    }

    void toByteArray(const std::string& data , ByteArray& bytes){
        toByteArray(data.c_str(),data.size(),bytes);
    }
